...
public class JavaCustomReceiver extends Receiver<String> {
	...
	public static void main(String[] args) {
		...
		SparkConf sparkConf = new SparkConf().setMaster("local[3]").setAppName("JavaCustomReceiver");
		JavaStreamingContext ssc = new JavaStreamingContext(sparkConf, new Duration(1000));

		// Create a input stream with the custom receiver on target ip:port and
		// count the
		// words in input stream of \n delimited text (eg. generated by 'nc')
		JavaReceiverInputDStream<String> lines = ssc.receiverStream(new JavaCustomReceiver("localhost", 5055));
	...
	private void receive() {
		try {
			...
			try {
				...
				InputStream myReader = socket.getInputStream();
				reader = new DataInputStream(myReader);
				// Until stopped or connection broken continue reading

				while ((userInput = reader.readUTF()) != null) {
